...
function Messages({ user}: Props){
   ...
   function createMessage(text : string){
    ...
    .then(resp=> resp.json())
    .then(newMessage => {
        const currentCopyMessage = JSON.parse(
    ...
    useEffect( () => {
    fetch(`http://localhost:8000/messages/${Number(params.conversationId)}`).then(resp => resp.json())
    .then(data => {
setGetMessages(data)
    })
    }, [])
    ...
    return(
        <div className="conversation">
           ...
           <ul className="conversation-messages">
               {
                  getMessages.map(message => <TextMessage message={message} outgoing={message.userId === user?.id} />)
               }
           </ul>
           <form  onSubmit={(e)=> {
               e.preventDefault()
